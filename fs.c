...
int dir_findname(struct fs_inode *dir_inode, char *name)
{
    ...
    while (remaining_dirents > 0)
    {
        ...
        {
            if (block.dirent[d].d_ino != FREE && strncmp(block.dirent[d].d_name, name, MAXFILENAME) == 0)
...
int get_inode(char *path_name)
{
    ...
    // Using while instead of do-while to handle empty paths like "////"
    while (token != NULL)
    {
...
char *get_filename(char *pathname)
{
    ...
    if (filename[0] == '\0')
        return NULL; // Path ends with '/', no filename
...
int add_entry_to_directory(int parent_ino, char *name, int child_ino)
{
    ...
    for (int i = 0; i < DIRBLOCK_PER_INODE; i++)
    {
        ...
        if (parent_inode.size > 0)
        {
            ...
            int entries_before_this_block = i * DIRENTS_PER_BLOCK;
            entries_in_block = total_entries - entries_before_this_block;
            if (entries_in_block < 0)
                entries_in_block = 0;
            if (entries_in_block > DIRENTS_PER_BLOCK)
                entries_in_block = DIRENTS_PER_BLOCK;
        }
    ...
    for (int i = 0; i < BLOCKSZ / sizeof(uint16_t); i++)
    {
        ...
        // Check space in this indirect block
        int total_entries = parent_inode.size / sizeof(struct fs_dirent);
        int entries_before_this_block = (DIRBLOCK_PER_INODE + i) * DIRENTS_PER_BLOCK;
...
int fs_mount(char *device, int size)
{
    ...
    if (disk_init(device, size) < 0)
        return -1; // open disk image or create if it does not exist
    disk_read(SBLOCK, block.data);
